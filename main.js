...
async function performScraping(searchString, scrapingType, folderPath, win, headless) {
...
ipcMain.handle('start-scraping', async (event, searchString, scrapingType, folderPath, headless) => {
    console.log(`Avvio dello scraping per: ${searchString} (${scrapingType}), headless: ${headless}`);
    const win = BrowserWindow.getAllWindows()[0];
    win.webContents.send('status', 'Inizio dello scraping...');
    await performScraping(searchString, scrapingType, folderPath, win, headless);
});
...
async function scrapeGoogleMaps(searchString, browser, win) {
    ...
    const acceptAllButton = await page.$('button[aria-label="Accept all"], button[aria-label="Accetta tutto"]');
    if (acceptAllButton) {
    ...
    const getPageData = async () => {
        ...
        win.webContents.send('status', `[data] Scritti ${cards.length} record, continuando alla prossima pagina se disponibile`);
        ...
        if (nextButton) {
            try {
                ...
                win.webContents.send('status', `[!] Errore clic su pagina successiva: ${e.message}`);
            }
        }
    };
    await getPageData();
...
async function saveMapsData(data, start_time, folderPath, win, searchQueries) {
    ...
    let queriesStr = searchQueries.join('_').replace(/\s+/g, '_').replace(/[^a-zA-Z0-9_]/g, '');
    ...
    win.webContents.send('status', `[success] Scritti ${data.length} record in ${(Date.now() - start_time.getTime()) / 1000}s`);
}
...
async function scrapePeopleAlsoAsk(searchString, browser, win, existingPage = null) {
    ...
    if (await checkForCaptcha(page)) {
        win.webContents.send('user-action-required', "[!] CAPTCHA rilevato! Risolvilo manualmente nel browser, poi clicca 'Continua' per proseguire.");
    ...
    while (results.length < maxToProcess) {
        ...
        for (let i = 0; i < faqContainers.length; i++) {
            ...
            try {
                ...
                win.webContents.send('status', `[success] Descrizione trovata: ${description ? description.slice(0, 80) + "..." : "[vuota]"}`);
